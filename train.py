...
parser.add_argument('--LOG_DIR', 
    default='/home/zwz/zwz/wck/PA-CVPR-MLP/DFML_logs',
...
parser.add_argument('--readpath', default='', type=str, help="")
...
if args.model.find('googlenet')+1:
    ...
    checkpoint = torch.load(args.readpath, map_location='cpu')
...
if args.loss == 'Proxy_Anchor':
    ...
    criterion = losses.MarginLoss(nb_classes = nb_classes).cuda()
elif args.loss == 'NTXent':
    criterion = losses.NTXentLoss().cuda()
...
if args.loss == 'Proxy_Anchor':
    ...
    param_groups.append({'params': criterion.parameters()})
...
for epoch in range(0, args.nb_epochs):
    ...
    if args.warm > 0:
        if args.gpu_id != -1:
            ...
            unfreeze_model_param = list(criterion.parameters())
            for i in range(12):
                unfreeze_model_param = unfreeze_model_param + list(model.module.blocks[i].attn.gate.parameters()) + list(model.module.blocks[i].gate.parameters())
    ...
    for batch_idx, (x, y) in pbar:                         
        m1, m2, x_ori_d_attn, x_hard_d_attn, x_ori_d_mlp, x_hard_d_mlp, prob_attn, prob_mlp = model(x.squeeze().cuda())
        distillation_loss = 0.1*(m2 - m1.detach()).pow(2).sum(dim=1).sqrt().sum()
        for i in range(12):
            distillation_loss = distillation_loss + 0.00001*(x_hard_d_attn[i]-x_ori_d_attn[i]).pow(2).sum(dim=2).sqrt().sum()/12 + 0.00001*(x_hard_d_mlp[i]-x_ori_d_mlp[i]).pow(2).sum(dim=2).sqrt().sum()/12
        loss = criterion(m1, y.squeeze().cuda()) + 0.5*criterion(m2, y.squeeze().cuda()) + 0.5*distillation_loss
