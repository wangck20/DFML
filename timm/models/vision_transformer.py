...
class Attention(nn.Module):
    ...
    def forward(self, x, signal):
        ...
        if signal == 'ori':
        	...
        	x = x1 * prob_true[:,:, 0].view(B, N, 1) + x2 * prob_true[:,:, 1].view(B, N, 1) + x3 * prob_true[:,:, 2].view(B, N, 1)
        	x_d = x1.detach() * prob_true_d[:,:, 0].view(B, N, 1) + x2.detach() * prob_true_d[:,:, 1].view(B, N, 1) + x3.detach()* prob_true_d[:,:, 2].view(B, N, 1)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        # x_hard = x_hard + self.drop_path1(self.ls1(self.attn(self.norm1(x_hard), signal = 'hard')))

        B, N, C = x_ori.shape
        x_ori = x_ori + self.drop_path2(self.ls2(self.mlp1(self.norm2(x_ori)))) + self.drop_path2(self.ls2(self.mlp2(self.norm2(x_ori)))) + self.drop_path2(self.ls2(self.mlp3(self.norm2(x_ori))))
        ...
        prob_hard = torch.nn.functional.one_hot(torch.argmax(prob_soft, dim=-1), 3)
        prob_true = (prob_hard - prob_soft).detach() + prob_soft
        prob_d = self.gate(x_hard.detach())
        prob_soft_d = torch.nn.functional.softmax(prob_d, dim=-1)
        prob_hard_d = torch.nn.functional.one_hot(torch.argmax(prob_soft_d, dim=-1), 3)
        prob_true_d = (prob_hard_d - prob_soft_d).detach() + prob_soft_d

        x_hard = x_hard + self.drop_path2(self.ls2(self.mlp1(self.norm2(x_hard))))*prob_true[:, :, 0].view(B, N, 1) + self.drop_path2(self.ls2(self.mlp2(self.norm2(x_hard))))*prob_true[:, :, 1].view(B, N, 1) + self.drop_path2(self.ls2(self.mlp3(self.norm2(x_hard))))*prob_true[:, :, 2] .view(B, N, 1)
        x_hard_d_mlp = x_hard.detach() + self.drop_path2(self.ls2(self.mlp1(self.norm2(x_hard.detach()))))*prob_true[:, :, 0].view(B, N, 1) + self.drop_path2(self.ls2(self.mlp2(self.norm2(x_hard.detach()))))*prob_true[:, :, 1].view(B, N, 1) + self.drop_path2(self.ls2(self.mlp3(self.norm2(x_hard.detach()))))*prob_true[:, :, 2] .view(B, N, 1)

        return [x_ori, x_hard], [x_ori_d_attn, x_hard_d_attn], [x_ori_d_mlp, x_hard_d_mlp], [prob_attn, torch.argmax(prob_true,dim=-1).view(-1,1)]
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            for i in range(self.depth):
                x, x_d_attn, x_d_mlp, prob = self.blocks[i](x)
                if i ==0:
                    prob_attn = prob[0]
                    prob_mlp = prob[1]
                else:
                    prob_attn = torch.cat((prob_attn, prob[0]),dim=1)
                    prob_mlp = torch.cat((prob_mlp, prob[1]),dim=1)
                x_ori_d_attn.append(x_d_attn[0])
                x_hard_d_attn.append(x_d_attn[1])
                x_ori_d_mlp.append(x_d_mlp[0])
                x_hard_d_mlp.append(x_d_mlp[1])
                #prob_attn.append(prob[0].sum(dim=0))
                #prob_mlp.append(prob[1].sum(dim=0))
            x_ori, x_hard = x[0], x[1]
        x_ori = self.norm(x_ori)
        x_hard = self.norm(x_hard)
        return x_ori, x_hard, x_ori_d_attn, x_hard_d_attn, x_ori_d_mlp, x_hard_d_mlp, prob_attn, prob_mlp
    ...
    def forward(self, x):
        x_ori, x_hard, x_ori_d_attn, x_hard_d_attn, x_ori_d_mlp, x_hard_d_mlp, prob_attn, prob_mlp = self.forward_features(x)
        x_ori = self.forward_head(x_ori)
        x_hard = self.forward_head(x_hard)
        if self.training:
        	return self.l2_norm(x_ori), self.l2_norm(x_hard), x_ori_d_attn, x_hard_d_attn, x_ori_d_mlp, x_hard_d_mlp, prob_attn, prob_mlp
        else:
