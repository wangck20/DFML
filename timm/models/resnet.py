...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.q = nn.Linear(dim, 1, bias=qkv_bias)
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x = x.reshape(B, C, H*W).reshape(B, H*W, C)

        q = self.q(x).softmax(dim = 1)
        #print(q.size())
        v = self.v(x)
        #v = x
        # q = self.attn_drop(q)
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.attention1 = Attention(dim = 2048)
        self.attention2 = Attention(dim = 2048)
    ...
    def forward_head(self, x, pre_logits: bool = False):
        comp1, q1 = self.attention1(x)
        comp2, q2 = self.attention2(x)
        #x = x.detach()
        x = self.global_pool(x)
        if self.drop_rate:
            x = F.dropout(x, p=float(self.drop_rate), training=self.training)
        if pre_logits:
            return x, comp1, comp2, q1, q2
        else:
            return self.fc(x), comp1, comp2, q1, q2
        
    def forward(self, x):
        x = self.forward_features(x)
        x, comp1, comp2, q1, q2 = self.forward_head(x)
        return x, comp1, comp2, q1, q2
